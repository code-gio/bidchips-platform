<script lang="ts">
  ...
  interface Props {
    open?: boolean;
    categories?: Category[];
    onConfirm: (data: Partial<Category>) => Promise<void>;
    onCancel?: () => void;
  }
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      errors.submit = error.message || "Failed to create category";
    } finally {
      isSubmitting = false;
    }
  }
...
<Drawer.Root bind:open>
  <Drawer.Content class="sm:max-w-md">
    <Drawer.Header>
      ...
      <Drawer.Description>
        Add a new category to organize your auction lots
      </Drawer.Description>
    </Drawer.Header>
        ...
        <FieldContent>
          ...
          <Select.Root bind:selected={parentId} onSelectedChange={(v) => {
            ...
            <Select.Content>
              ...
              {#each categories.filter((c) => c.id !== parentId) as cat}
