...
export const PUT: RequestHandler = async (event) => {
  try {
    ...
    if (offer.status !== "pending") {
      return errorResponse("Only pending offers can be rejected", 400, "OFFER_NOT_PENDING");
    }
    ...
    await supabaseAdmin.from("notifications").insert({
      ...
      message: `Your offer on "${offer.lot_title}" has been rejected`,
      action_url: `/lots/${offer.lot_id}`,
    });
    ...
    await supabaseAdmin.from("activity_log").insert({
      ...
      resource: "offer",
      resource_id: offerId,
      description: `Rejected offer on "${offer.lot_title}"`,
